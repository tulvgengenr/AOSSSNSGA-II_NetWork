 ...
 * </ol>
 */
public class DifferentialEvolution3 implements Variation,Serializable {
	 ...
	 */
	public DifferentialEvolution3(double CR, double F, double K) {
		this.CR = CR;
		this.F = F;
                this.K = K;
		...
		for (int j = 0; j < result.getNumberOfVariables(); j++) {
			if ((pprng.nextDouble() <= CR) || (j == jrand)) {
				...
				RealVariable v3 = (RealVariable)parents[3].getVariable(j);
                                ...
                                RealVariable v6 = (RealVariable)parents[6].getVariable(j);
