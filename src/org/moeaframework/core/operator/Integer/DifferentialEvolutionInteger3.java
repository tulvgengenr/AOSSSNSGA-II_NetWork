 ...
 * </ol>
 */
public class DifferentialEvolutionInteger3 implements Variation,Serializable {
    private static final long serialVersionUID = -1476550247134544331L;
	 ...
	 */
	public DifferentialEvolutionInteger3(double CR, double F, double K) {
		this.CR = CR;
		this.F = F;
                this.K = K;
	...
	public Solution[] evolve(Solution[] parents) {
		...
		for (int j = 0; j < result.getNumberOfVariables(); j++) {
			if ((pprng.nextDouble() <= CR) || (j == jrand)) {
				IntegerVariable v0 = (IntegerVariable)result.getVariable(j);
				IntegerVariable v1 = (IntegerVariable)parents[1].getVariable(j);
				IntegerVariable v2 = (IntegerVariable)parents[2].getVariable(j);
				IntegerVariable v3 = (IntegerVariable)parents[3].getVariable(j);
				IntegerVariable v4 = (IntegerVariable)parents[4].getVariable(j);
				IntegerVariable v5 = (IntegerVariable)parents[5].getVariable(j);
				IntegerVariable v6 = (IntegerVariable)parents[6].getVariable(j);

				double y = v6.getValue() + K * (v6.getValue() - v1.getValue()) + F * (v2.getValue() - v3.getValue()) + F * (v4.getValue() - v5.getValue());
