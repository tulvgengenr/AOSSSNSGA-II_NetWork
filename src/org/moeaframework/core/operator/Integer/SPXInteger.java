 ...
 * </ol>
 */
public class SPXInteger implements Variation,Serializable {
    private static final long serialVersionUID = -3491545703712381908L;
	 ...
	 */
	public SPXInteger(int numberOfParents, int numberOfOffspring) {
	 ...
	 */
	public SPXInteger(int numberOfParents, int numberOfOffspring, double epsilon) {
	...
	public Solution[] evolve(Solution[] parents) {
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				G[j] += ((IntegerVariable)parents[i].getVariable(j)).getValue();
			}
		}
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				x[i][j] = G[j]
						+ epsilon
						* (((IntegerVariable)parents[i].getVariable(j)).getValue() - G[j]);
			}
		}
		...
		for (int k = 0; k < numberOfOffspring; k++) {
			...
			for (int j = 0; j < m; j++) {
				IntegerVariable variable = (IntegerVariable)child.getVariable(j);
    ...
    public String toString() {
        return "SPXInteger{" + "#Parents=" + numberOfParents + ", #Offspring=" + numberOfOffspring + ", eps=" + epsilon + '}';
    }
