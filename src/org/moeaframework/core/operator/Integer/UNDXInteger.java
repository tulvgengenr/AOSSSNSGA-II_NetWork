 ...
 * </ol>
 */
public class UNDXInteger implements Variation,Serializable{
    private static final long serialVersionUID = -5436223338286185449L;
	 ...
	 *        operator
	 */
	public UNDXInteger(int numberOfParents, int numberOfOffspring) {
	 ...
	 */
	public UNDXInteger(int numberOfParents, int numberOfOffspring, double zeta,
			double eta) {
	...
	public Solution undx(Solution[] parents) {
		...
		for (int i = 0; i < k; i++) {
			for (int j = 0; j < n; j++) {
				x[i][j] = ((IntegerVariable)parents[i].getVariable(j)).getValue();
			}
		}
		...
		for (int j = 0; j < n; j++) {
			IntegerVariable variable = (IntegerVariable)result.getVariable(j);
    ...
    public String toString() {
        return "UNDXInteger{" + "#Parents=" + numberOfParents + ", #Offspring=" + numberOfOffspring + ", zeta=" + zeta + ", eta=" + eta + '}';
    }
}
